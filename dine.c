...
/*   Updated: 2025/12/09 15:10:08 by nismayil         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void eat(t_philo *philo)
{
    ...
    if (philo->props->n_must_eat > 0 && philo->times_ate == philo->props->n_must_eat)
...
void run_sim(t_props *props)
{
    ...
    if (props->number_of_philosophers == 1)
        pthread_create(&props->philos[0].thread, NULL, lone_philo, (void *)&props->philos[0]);
    else
        while (++i < props->number_of_philosophers)
            pthread_create(&props->philos[i].thread, NULL, dine, (void *)&props->philos[i]);
    pthread_create(&props->monitor, NULL, track, (void *)props);
    props->start_time = get_time(MILLISECOND);
